...
void setup() {
  ...
  // PWM TCA Output Channels
  PORTB.DIRSET = PIN0_bm;  // D9, PB0 TCA_CMP0
  PORTB.DIRSET = PIN1_bm;  // D10, PB1 TCA_CMP1
  PORTB.DIRSET = PIN2_bm;  // D5, PB2 TCA_CMP2

  // Timer A Setup
  TCA0.SINGLE.CTRLB = TCA_SINGLE_CMP2EN_bm | TCA_SINGLE_WGMODE_SINGLESLOPE_gc;  // Single slope and enable channel 0
  ...
  TCA0.SINGLE.CMP2BUF = boost_period_ticks * boost_duty;  // Set the pulse width
...
ISR(TCA0_OVF_vect) {
  if (boost_count < boost_pulse_count_quickstart_on) {
    TCA0.SINGLE.CMP2BUF = boost_period_ticks;  // Set to constant on for quick start
